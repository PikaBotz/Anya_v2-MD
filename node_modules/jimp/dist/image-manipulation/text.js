...
function printText(font, x, y, text) {
  for (var i = 0; i < text.length; i++) {
    ...
    x += (font.kernings[char] && font.kernings[char][text[i + 1]] ? font.kernings[char][text[i + 1]] : 0) + (font.chars[char].xadvance || 0);
  }
}
/**
...
function print(font, x, y, text, maxWidth, maxHeight, cb) {
  ...
  if (_typeof(font) !== 'object') {
    return _errorChecking.throwError.call(this, 'font must be a Jimp loadFont', cb);
  }

  if (typeof x !== 'number' || typeof y !== 'number' || typeof maxWidth !== 'number') {
    return _errorChecking.throwError.call(this, 'x, y and maxWidth must be numbers', cb);
  }

  if (typeof text !== 'string' && _typeof(text) !== 'object') {
    return _errorChecking.throwError.call(this, 'text must be a string or an object', cb);
  }

  if (typeof maxWidth !== 'number') {
    return _errorChecking.throwError.call(this, 'maxWidth must be a number', cb);
  }

  if (typeof maxHeight !== 'number') {
    return _errorChecking.throwError.call(this, 'maxHeight must be a number', cb);
  }
  ...
  if (maxHeight !== Infinity && alignmentY === constants.VERTICAL_ALIGN_BOTTOM) {
    y = maxHeight - measureTextHeight(font, text, maxWidth);
  } else if (maxHeight !== Infinity && alignmentY === constants.VERTICAL_ALIGN_MIDDLE) {
  ...
  for (var n = 0; n < words.length; n++) {
    ...
    if (testWidth > maxWidth && n > 0) {
      this.print(font, x + xOffsetBasedOnAlignment(font, line, maxWidth, alignmentX), y, line);
      line = words[n] + ' ';

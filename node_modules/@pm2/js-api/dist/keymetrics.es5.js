...
},{"_process":35,"timers":36}],34:[function(require,module,exports){
/**
 * Helpers.
 */
...
module.exports = /*#__PURE__*/function (_AuthStrategy) {
  ...
  var _super = _createSuper(BrowserFlow);
    ...
    value: function retrieveTokens(km, cb) {
      ...
      if (params.get('access_token') !== null) {
        ...
        verifyToken(params.get('access_token')).then(function (res) {
          _this.removeUrlToken(res.data.refresh_token); // Save refreshToken in localstorage
...
},{"./strategy":41}],40:[function(require,module,exports){
'use strict';
...
module.exports = /*#__PURE__*/function (_AuthStrategy) {
  ...
  var _super = _createSuper(StandaloneFlow);
...
},{"./strategy":41}],41:[function(require,module,exports){
'use strict';
...
},{"../../constants.js":1,"./browser_strategy":39,"./embed_strategy":30,"./standalone_strategy":40}],42:[function(require,module,exports){
'use strict';
...
},{"./utils/validator":46,"debug":31}],43:[function(require,module,exports){
'use strict';
...
var Keymetrics = /*#__PURE__*/function () {
  ...
  */
  function Keymetrics(opts) {
    ...
    logger("Using mappings provided in ".concat(opts && opts.mappings ? 'options' : 'package')); // build namespaces at startup
  ...
  _createClass(Keymetrics, [{
    ...
    value: function use(flow, opts) {
      ...
      this._network.useStrategy(flow, opts); // the logout is dependent of the auth flow so we need it to be initialize
...
},{"../constants":1,"./api_mappings.json":38,"./namespace":44,"./network":45,"debug":31}],44:[function(require,module,exports){
'use strict';
    ...
    for (var name in mapping) {
      ...
      if (_typeof(mapping) === 'object' && !child.route) {
        ...
        if (name === 'default') {
          ...
          this.namespaces.push(defaultNamespace); // bind property of the default namespace to this namespace
...
},{"./endpoint":42,"debug":31}],45:[function(require,module,exports){
'use strict';
  ...
  _createClass(NetworkWrapper, [{
    ...
    value: function _setupDateLag() {
      ...
      var updateApiDateLag = function updateApiDateLag(response) {
        if (response && response.headers && response.headers.date) {
          ...
          var clientDate = new Date(); // The header date is likely to be truncated to the second, so truncate the client date too
    ...
    value: function _queueUpdater() {
      ...
      while (this._queue.length > 0) {
        var promise = this._queue.shift(); // make the request
    ...
    value: function request(httpOpts) {
      ...
      return new Promise(function (resolve, reject) {
        async.series([// verify that we don't need to buffer the request because authentication
        function (next) {
        ...
        function (next) {
          if (!httpOpts.url.match(/bucket\/[0-9a-fA-F]{24}/)) return next(); // parse the bucket id from URL

          var bucketID = httpOpts.url.split('/')[3]; // we need to retrieve where to send the request depending on the backend

          _this3._resolveBucketEndpoint(bucketID).then(function (endpoint) {
            httpOpts.baseURL = endpoint; // then continue the flow
        ...
        function (next) {
          ...
          _this3._axios.request(httpOpts).then(successNext)["catch"](function (error) {
            var response = error.response; // we only need to handle when code is 401 (which mean unauthenticated)

            if (response && response.status !== 401) return next(response);
            loggerHttp("Got unautenticated response, buffering request from now ..."); // we tell the client to not send authenticated request anymore
    ...
    value: function _updateTokens(err, data, cb) {
      ...
      if (err) {
        console.error('Error while retrieving tokens:', err); // Try to logout/login user
      ...
      this._axios.defaults.headers.common['Authorization'] = "Bearer ".concat(data.access_token);
    ...
    value: function useStrategy(flow, opts) {
      if (!opts) opts = {}; // if client not provided here, use the one given in the instance
      ...
      var flowMeta = AuthStrategy.implementations(flow); // verify that the environnement condition is meet
    ...
    value: function subscribe(bucketId, opts) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this5.km.bucket.retrieve(bucketId).then(function (res) {
          ...
          loggerWS("Found endpoint for ".concat(bucketId, " : ").concat(endpoint)); // connect websocket client to the realtime endpoint
...
},{"../constants":1,"./auth_strategies/strategy":41,"./utils/websocket":47,"async":2,"axios":3,"debug":31,"eventemitter2":33}],46:[function(require,module,exports){
'use strict';
          ...
          case 'GET':
            {
              ...
              try {
                for (_iterator.s(); !(_step = _iterator.n()).done;) {
                  ...
                  var value = args.shift(); // params should always be a string since they will be replaced in the url
              ...
              try {
                for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
                  ...
                  var _value = args.shift(); // query should always be a string since they will be replaced in the url
          ...
          case 'PATCH':
            {
              ...
              try {
                for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
                  ...
                  var _value2 = args.shift(); // params should always be a string since they will be replaced in the url
              ...
              try {
                for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
                  ...
                  var _value3 = args.shift(); // query should always be a string since they will be replaced in the url
              ...
              try {
                for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
                  ...
                  var isSubfield = field.name.includes('[]') === true; // verify that the mandatory field are here
          ...
          case 'DELETE':
            {
              ...
              try {
                for (_iterator6.s(); !(_step6 = _iterator6.n()).done;) {
                  ...
                  var _value4 = args.shift(); // params should always be a string since they will be replaced in the url
              ...
              try {
                for (_iterator7.s(); !(_step7 = _iterator7.n()).done;) {
                  ...
                  var _value5 = args.shift(); // query should always be a string
...
ReconnectableWebSocket.prototype._onerror = function (event) {
  debug('onerror', event); // To avoid undetermined state, we close socket on error
...
},{"debug":31,"ws":30}],"/":[function(require,module,exports){
"use strict";

module.exports = require('./src/keymetrics.js');

},{"./src/keymetrics.js":43}]},{},[])("/")
});

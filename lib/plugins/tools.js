...
anya(
  ...
  async (anyaV2, pika, { args, prefix, command }) => {
    ...
    const input = pika.quoted ? pika.quoted?.text ? pika.quoted.text.toLowerCase() : args.join(" ").toLowerCase() : args.join(" ").toLowerCase();
    ...
    if (!/^--[a-z]{2,}(-[a-z]{2,})?$/i.test(input.trim())) {
      ...
      .then(response => {
        ...
        return pika.reply("*🤖 Explanation :* " + response.message + footer);
      });
        ...
        if (languages.hasOwnProperty(match[1])) {
          ...
          language = languages[match[1]];
            ...
            //options = { to: "en" };
          language = "English";
          header = "> _❗ language prefix \"" + match[1] + "\" not found!_\n\n";
        }
        chatGpt4(pika.sender.split("@")[0], "hey bro can you tell me intensions of this to message in " + language + "? " + cleanedUp + `"`)
        .then(response => {
           if (!response.status || !response.message) return pika.reply("_Ai is not available right now!_");
           return pika.reply("*🤖 Explanation :* " + response.message + (header ? header : footer));
        });
        ...
        .then(response => {
            return pika.reply("*🌐 Translation :* " + response + (header ? header : footer));
        });
    }
  });

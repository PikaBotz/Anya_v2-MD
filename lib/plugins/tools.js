...
const { anya, listall, UI } = require('../lib');
...
anya(
    ...
    const [packname = Config.packname, author = Config.author] = args.length > 0 
        ? args.join(" ").split('|').map(item => item.trim())
        : [];
    const media = await quoted.download();
    switch (true) {
        case /webp/.test(mime):
            await anyaV2.sendMessage(pika.chat, { sticker: media }, pika, { packname, author });
            break;

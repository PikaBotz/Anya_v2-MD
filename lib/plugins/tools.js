...
anya(
  ...
  async (anyaV2, pika, { args, prefix, command }) => {
    if (args.length < 1 && !pika.quoted?.text) return pika.reply(
      "_Enter or quote a message_"
    ...
    const input = pika.quoted
    ? pika.quoted?.text
      ? pika.quoted.text
      : args.join(" ");
    : args.join(" ");
    const regex = /--(\w+)/;
    const isLangPrefix = input.match(regex) || false;
    let from;
    let to = "en";
    const translate = require('translate-google');
    const languages = require('../database/json/languages');
    const footer = "\n\n> " + Config.footer;
    if (!isLangPrefix) {
      translate(input, { to })
      .then(response => {
        return pika.reply(response + footer);
      });
    } else if (input.match("--ai")) {
      chatGpt4(pika.sender.split("@")[0], "Hey! Can you tell me what this person want to say? want's his intensions after saying this, please tell me in english language: " + input)
      .then(response => {
      ...
      const match = input.match(/--ai-(\w+)/);
      const afterFix = match[0].toLowerCase();
      to = languages[afterFix];
      if (!to) to = "English";
      chatGpt4(pika.sender.split("@")[0], "Hey! Can you tell me what this person want to say? want's his intensions after saying this, please tell me in " + to + " language: " + input)
      .then(response => {
        if (!response.status || !response.message) return pika.reply("_Ai assistant isn't available right now!_");
        return pika.reply("> ⚠️ No Valid Language Option Found!\n\n" + response.message + footer);
      });
    }
  }
)

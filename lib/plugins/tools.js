...
anya(
  ...
  async (anyaV2, pika, { args, prefix, command }) => {
    ...
    const input = pika.quoted ? pika.quoted.text ? pika.quoted.text.toLowerCase() : args[0] ? text.toLowerCase() : false : args[0] ? text.toLowerCase() : false;
    ...
    if (!/--[a-z]{2,}(-[a-z]{2,})?/i.test(input)) { console.log("Using No detection");
      ...
      const cheanedUp = input.replace(/--[a-z]+-[a-z]+\b/i, "").trim();

...
anya(
  ...
  async (anyaV2, pika, { args, prefix, command }) => {
    if (args.length < 1 && !pika.quoted?.text) return pika.reply("_Enter or quote a message_" + "\n\n*Don't know how to use this plugin's full powers❓*" + "\n> Click here : https://github.com/PikaBotz/Anya_v2-MD/wiki/Translate-Plugin-Usage-Guide");
    const input = pika.quoted ? pika.quoted?.text ? pika.quoted.text : args.join(" ") : args.join(" ");
    const regex = /--(\w+)/g;
    ...
    const footer = "\n\n> " + Config.footer;
    const hasAi = input.includes('--ai');
    let aiLang = null;
    let cleanedInput = input.replace(regex, '').trim();
    if (match) {
      match = match.map(x => x.replace('--', '').toLowerCase());
      if (match.includes('ai')) {
        aiLang = match.find(lang => lang !== 'ai');
        match = match.filter(lang => lang !== 'ai');
      }
      if (match.length === 1) {
        to = languages[match[0]] || getClosestLang(match[0], languages) || "en";
      } else if (match.length === 2) {
        from = languages[match[0]] || getClosestLang(match[0], languages);
        to = languages[match[1]] || getClosestLang(match[1], languages) || "en";
      } else {
        from = null;
        to = "en";
      }
    ...
    if (aiLang) {
      let promptLang = languages[aiLang] || getClosestLang(aiLang, languages) || "English";
      chatGpt4(pika.sender.split("@")[0], `Please translate the following and understand the intention in ${promptLang} language: ` + cleanedInput)
      .then(response => {
        if (!response.status || !response.message) return pika.reply("_Ai assistant isn't available right now!_");
        return pika.reply(response.message + footer);
      });
      return;
    }
    if (!from) {
      translate(cleanedInput, { to })
      .then(response => pika.reply(response + footer))
      .catch(err => pika.reply("> ❗ Input language prefix not found!\n\n" + err.message + footer));
    } else {
      translate(cleanedInput, { from, to })
      .then(response => pika.reply(response + footer))
      .catch(err => pika.reply("> ❗ Error during translation!\n\n" + err.message + footer));
    }
  }
);

function getClosestLang(prefix, languages) {
  const keys = Object.keys(languages);
  let closestMatch = keys.find(lang => lang.startsWith(prefix));
  return closestMatch ? languages[closestMatch] : null;
}

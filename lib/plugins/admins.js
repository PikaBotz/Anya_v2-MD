...
anya({ name: "gcrequest", alias: ['grouprequest', 'joinrequest', 'joinrequests'], react: "👥", category: "admins", desc: "See all group joining requests.", rule: 3, cooldown: 8, filename: __filename
        ...
        if (ui.buttons) {
            ...
            const requestlist = await Promise.all(data.map(async r => 
                `{\"title\":\"${Config.themeemoji} ${await anyaV2.getName(r.jid)}\",\"rows\":[{\"header\":\"✅ Accept\",\"title\":\"\",\"description\":\"Accept the user's request\",\"id\":\"${prefix}accepteverygrouprequest ${r.jid}\"},{\"header\":\"❌ Decline\",\"title\":\"\",\"description\":\"Decline the user's request\",\"id\":\"${prefix}declineeverygrouprequest ${r.jid}\"}]}`
            )).join(",");

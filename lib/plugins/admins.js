...
anya({ name: "gcrequest", alias: ['grouprequest', 'joinrequest', 'joinrequests'], react: "👥", category: "admins", desc: "See all group joining requests.", rule: 3, cooldown: 8, filename: __filename
        ...
        if (ui.buttons) {
            ...
            const requestlist = data.map(async r => `{\"title\":\"${Config.themeemoji} ${await anyaV2.getName(r.jid)}\",\"highlight_label\":\"${Config.botname}\",\"rows\":[{\"header\":\"✅ Accept\",\"title\":\"\",\"description\":\"accept the user's request\",\"id\":\"${prefix}accepteverygrouprequest ${r.jid}\"},{\"header\":\"❌ Decline\",\"title\":\"\",\"description\":\"decline the user's request\",\"id\":\"${prefix}declineverygrouprequest ${r.jid}\"}]}`).join(",");
console.log(requestlist)
          return await anyaV2.sendButtonText(pika.chat, {
                ...
                buttons: [{
                    "name": "single_select",
                    "buttonParamsJson": `{\"title\":\"Pending Requests 🧾\",\"sections\":[{\"title\":\"⚡ 𝗥𝗲𝗾𝘂𝗲𝘀𝘁 𝗟𝗶𝘀𝘁 ⚡\",\"highlight_label\":\"${Config.botname}\",\"rows\":[{\"header\":\"🍇 Accept All Requests 🍇\",\"title\":\"\",\"description\":\"Click here to accept all requests\",\"id\":\"${prefix}accepteverygrouprequest ${requests}\"}]},{\"title\":\"⚡ 𝗥𝗲𝗾𝘂𝗲𝘀𝘁 𝗟𝗶𝘀𝘁 ⚡\",\"highlight_label\":\"${Config.botname}\",\"rows\":[${pluginlist}]}]}`
                },
                {
                    "name": "quick_reply",

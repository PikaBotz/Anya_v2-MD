...
const { delay, anya, getBuffer, fancy13, Group, announce, numberToDate } = require('../lib');
...
anya({
        name: "gcrequest",
        alias: ['grouprequest', 'joinrequest', 'joinrequests'],
        react: "👥",
        category: "admins",
        desc: "See all group joining requests.",
        rule: 3,
        cooldown: 8,
        filename: __filename
}, async (anyaV2, pika) => {
    const response = await anyaV2.groupRequestParticipantsList(pika.chat);
    if (response.length < 1) return pika.reply(`❌No Requests Pending.`);
    const mentions = [];
    let c = 1;
    let reply = `\`👤 Group Joining Requests!\`\n
*Reply Number:*
- _Reply 0 to decline every request_
- _Reply 00 to accept every request_
- _Reply an specific number to accept_
═══════════════════════\n\n`;
    for (let i of response) {
        mentions.push(i.jid);
        const timestamp = numberToDate(i.request_time);
        reply += `*${c++}.* @${i.jid.split("@")[0]}
> *📆Date :* ${timestamp.date}
> *🕜Time :* ${timestamp.time}\n\n`;
    }
    return pika.reply(`${reply}_ID: QA31_`, { mentions: mentions });
});

//༺─────────────────────────────────────༻

anya({
        name: "accepteverygrouprequest",
        react: "🪀",
        category: "admins",
        desc: "Internal Command For Accepting Joining Request.",
        rule: 3,
        notCmd: true,
        cooldown: 8,
        filename: __filename
...
anya({
        name: "declineeverygrouprequest",
        react: "🪀",
        category: "admins",
        desc: "Internal Command For Rejecting Joining Request.",
        rule: 3,
        notCmd: true,
        cooldown: 8,
        filename: __filename

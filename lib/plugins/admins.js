...
anya({ name: "gcrequest", alias: ['grouprequest', 'joinrequest', 'joinrequests'], react: "👥", category: "admins", desc: "See all group joining requests.", rule: 3, cooldown: 8, filename: __filename
        ...
        if (ui.buttons) {
            ...
            const requestlist = (await Promise.all(data.map(async r => {
                ...
                return `{\"title\":\"${Config.themeemoji} ${name}\",\"highlight_label\":\"${Config.botname}\",\"rows\":[{\"header\":\"✅ Accept\",\"title\":\"\",\"description\":\"accept the user's request\",\"id\":\"${prefix}accepteverygrouprequest ${r.jid}\"},{\"header\":\"❌ Decline\",\"title\":\"\",\"description\":\"decline the user's request\",\"id\":\"${prefix}declineverygrouprequest ${r.jid}\"}]}`;
            }))).join(",");

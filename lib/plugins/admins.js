...
anya({ name: "tagall", alias: ['tall'], react: "🎊", category: "admins", need: "text", desc: "Tag everyone in the group", rule: 3, filename: __filename }, async (anyaV2, pika, { args }) => {
        const tagm = [];
        const text = args.join(" ");
        const metadata = await anyaV2.groupMetadata(pika.chat);
        tagm.push(`*⛩️ Message :* \`${pika.quoted ? (pika.quoted.text.length > 1 ? pika.quoted.text : 'Empty message') : (args.length > 0 ? text : 'Empty message')}\`\n\n*🎏 Announcer :* @${pika.sender.split('@')[0]}\n┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n\n╭─⌈ 𝘼𝙙𝙢𝙞𝙣𝙨 ⌋`.trim());
        tagm.push(metadata.participants.filter(admins => admins.admin !== null).map(admins => `👑 @${admins.id.split('@')[0]}`));
        tagm.push("\n╭─⌈ 𝙈𝙚𝙢𝙗𝙚𝙧𝙨 ⌋");
        tagm.push(metadata.participants.filter(mem => mem.admin === null).map(mem => `👤 @${mem.id.split('@')[0]}`));
        const quoted = pika.quoted ? pika.quoted : pika;
        const mime = (quoted && quoted.mimetype) ? quoted.mimetype : pika;
        const buttons = [

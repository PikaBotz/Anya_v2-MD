    ...
    if (!args[0]) {
        if (ui.buttons) {
            ...
            const generateButtonList = () => {
                for (const [key, value] of Object.entries(types)) {
                    butArray.push(`{\"header\":\"${value}\",\"title\":\"${ui.reply === Number(key) ? fancy17("(selected)") : ""}\",\"description\":\"𝘵𝘢𝘱 𝘩𝘦𝘳𝘦 𝘁𝗼 𝘀𝗲𝗹𝗲𝗰𝘁\",\"id\":\"${prefix + command} ${key}\"}`);
                }
                return butArray;
            };
            generateButtonList();
...
anya({
    ...
    if (!args[0]) {
        if (ui.buttons) {
            ...
            const generateButtonList = () => {
                for (const [key, value] of Object.entries(types)) {
                    butArray.push(`{\"header\":\"${value}\",\"title\":\"${ui.menu === Number(key) ? fancy17("(selected)") : ""}\",\"description\":\"𝘵𝘢𝘱 𝘩𝘦𝘳𝘦 𝘁𝗼 𝘀𝗲𝗹𝗲𝗰𝘁\",\"id\":\"${prefix + command} ${key}\"}`);
                }
                return butArray;
            };
            generateButtonList();
...
anya({
    ...
    if (!args[0]) {
        if (ui.buttons) {
            ...
            const generateButtonList = () => {
                for (const [key, value] of Object.entries(types)) {
                    butArray.push(`{\"header\":\"${value}\",\"title\":\"${ui.alive === key ? fancy17("(selected)") : ""}\",\"description\":\"𝘵𝘢𝘱 𝘩𝘦𝘳𝘦 𝘁𝗼 𝘀𝗲𝗹𝗲𝗰𝘁\",\"id\":\"${prefix + command} ${key}\"}`);
                }
                return butArray;
            };
            generateButtonList();
...
anya({
    ...
    if (ui.buttons) {
        ...
        for (let i = 0; i < results.length; i++) {
            butArray.push(`{\"header\":\"Command ${i + 1}\",\"title\":\"${results[i].locked ? fancy17("(Locked)") : ""}\",\"description\":\"Tap here to select\",\"id\":\"${prefix + command} ${results[i].bash64}\"}`);
        }
        return await anyaV2.sendButtonText(pika.chat, {
            text: `
*Heya, @${pika.sender.split("@")[0]}! 👋🏻*

...
anya({
    ...
    const ui = db.UI[0] || (await new UI({ id: "userInterface" }).save()
        .then(async () => await ManageDB.reload()));
...
_Choose the option below to select listmenu message type._
`.trim(),
                footer: Config.footer,
                buttons: [{ "name": "single_select", "buttonParamsJson": `{\"title\":\"Tap Here 🔰\",\"sections\":[{\"title\":\"𝗖𝗵𝗼𝗼𝘀𝗲 𝗠𝗲𝗻𝘂 𝗧𝘆𝗽𝗲 𝗗𝗮𝗿𝗹𝗶𝗻𝗴 💓💲\",\"highlight_label\":\"${Config.botname}\",\"rows\":[${butArray.join(",")}]}]}` }],
                contextInfo: {
                    mentionedJid: [pika.sender]
                }
            }, { quoted: pika });
...
`.trim(), { mentions: [pika.sender], forwarded: false });
    ...
    const optionNum = Number(args[0]);
    if (!optionNum || !types[optionNum]) return pika.reply(`_❌ Oopsie! Invalid option type, Senpai. Choose a valid number!_`);
    if (optionNum === ui.listmenu) return pika.reply(`☑️ Menu message already set as *${types[optionNum]}*.`);
    await UI.updateOne({ id: "userInterface" }, { $set: { listmenu: optionNum } }, { new: true });
    return pika.reply(`✅ Successfully enabled \`${types[optionNum]}\` as *listmenu type*! 🎉`);
});

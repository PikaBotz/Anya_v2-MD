...
const { anya, delay, UI, Bot, System, Cmd, UI } = require('../lib');

//༺─────────────────────────────────────

anya({ name: "buttons", react: "🤍", category: "core", desc: "Enable buttons", rule: 1, filename: __filename
    }, async (anyaV2, pika, { args, prefix, command }) => {
        const ui = await UI.findOne({ id: "userInterface" }) || await new UI({ id: "userInterface" }).save();
        if (/on/.test(args[0])) {
            if (ui.buttons) {
                return pika.reply("_Already Enabled Buttons..!_");
            } else {
            ...
            if (!ui.buttons) {
                return pika.reply("_Already Disabled Buttons..!_");
            } else {

...
anya(
    ...
    async (anyaV2, pika, { args }, { boxKey }) => {
        ...
        await anyaV2.sendMessage(joinedGroup, { text: caption }, { quoted: botConQuote });
        await delay(1000);
...
anya(
    ...
    async (anyaV2, pika, { args }, { boxKey }) => {
        ...
        await pika.reply("*💖 Thanks for reporting a bug!*");
        const botConName = await anyaV2.getName(anyaV2.user.id);
        const botConQuote = {
            key: { participant: '0@s.whatsapp.net', remoteJid: 'status@broadcast' },
            message: {
...
anya(
	...
	async (anyaV2, pika) => {
		pika.reply("My Uptime : _" + formatRuntime(process.uptime()).trim() + "_");
	}
)

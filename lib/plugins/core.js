...
anya({
    ...
    if (ui.buttons) {
        ...
        for (let i = 0; i < results.length; i++) {
            butArray.push(`{\"header\":\"${results[i].locked ? "🔒" : ""} ${results[i].command}\",\"title\":\"${results[i].type}\",\"description\":\"${results[i].bash64}\",\"id\":\"${prefix}delcmd ${results[i].bash64}\"}`);
        }
        return await anyaV2.sendButtonText(pika.chat, {
            text: `
*${greetTime(2).greetingWithEmoji}, @${pika.sender.split("@")[0]}! 👋🏻*
...
`.trim(),
            footer: Config.footer,
            buttons: [{ "name": "single_select", "buttonParamsJson": `{\"title\":\"Select Command Hash 📝\",\"sections\":[{\"title\":\"𝗦𝗲𝗹𝗲𝗰𝘁 𝘁𝗼 𝗱𝗲𝗹𝗲𝘁𝗲 𝗕𝗮𝗸𝗮 📃\",\"highlight_label\":\"${Config.botname}\",\"rows\":[${butArray.join(",")}]}]}` }],
            contextInfo: {
                mentionedJid: [pika.sender]
            }
        }, { quoted: pika });

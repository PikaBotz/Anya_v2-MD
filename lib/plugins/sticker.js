...
anya(
    ...
    async (anyaV2, pika, { args }) => {
        ...
        const buffer = await getBuffer("https://api.erdwpe.com/api/maker/attp?text=" + encodeURIComponent(args.join(" ")));
...
anya(
    ...
    async (anyaV2, pika, { args }) => {
        if (!args[0]) return pika.reply("_Where's the text❓_");
        const buffer = await getBuffer("https://api.erdwpe.com/api/maker/ttp?text=" + encodeURIComponent(args.join(" ")));
        await anyaV2.sendVideoAsSticker(pika.chat, buffer, pika, { packname: Config.packname, author: Config.author });
        //pika.edit("> ✅ Created!", key);
    }
)

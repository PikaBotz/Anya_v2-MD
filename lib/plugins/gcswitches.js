    ...
    if (args.length < 1) {
        ...
        if (ui.buttons) {
            ...
            for (const i of dataArray) {
                butArray.push(`{\"header\":\"${i.emoji} ${i.name}\",\"title\":\"\",\"description\":\"${fancy13("tap here to " + command + " " + i.id)}\",\"id\":\"${prefix + command} ${i.id}\"}`);
            }
...
`.trim(),
            ...
            for (const i of dataArray) {
                caption += num++ + ". _" + i.name + "_ " + i.emoji + "\n";
            }
            caption += "> Tip: type `" + prefix + command + " <option>` to enable directly!\n";
            caption += "_ID: QA35_";
        ...
        announcement ? await announce(anyaV2, pika, { message: announcementTxt }) : null;
...
`.trim(),
            ...
            caption += "> Tip: type `" + prefix + command + " <option>` to disable directly!\n";
            caption += "_ID: QA36_";
            return await anyaV2.sendMessage(pika.chat, {
                image: await getBuffer(thumbUrl),
                caption: caption.trim()

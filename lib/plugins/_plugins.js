...
anya({ name: "plugins", alias: ['plugin'], react: "🚀", category: "core", desc: "See all plugins list", rule: 1, filename: __filename },
    ...
    if (ui.buttons) {
        const pluginlist = external.map((item, index) => `{\"header\":\"${index + 1} => ${item.id}.js\",\"title\":\"𝘤𝘭𝘪𝘤𝘬 𝘩𝘦𝘳𝘦 𝘁𝗼 𝘀𝗲𝘁 𝘁𝗵𝗲 𝗱𝗲𝗹𝗲𝘁𝗲\",\"description\":\"°•~━━✥ Queen Anya MD ✥━━~•°\",\"id\":\"${prefix}delplugins ${item.id}\"}`).join(',');
        const links = external.map(item => item.url).join(',');
        const caption = "`⧉ Plugins List ⧉`\n\n*🍉 Bot:* " + Config.botname + "\n*🍓 External Plugins:* " + external.length + " installed";
        return await anyaV2.sendButtonImage(pika.chat, {
            ...
            buttons: [{
                "name": "single_select",
                "buttonParamsJson": `{\"title\":\"See Plugins 🧾\",\"sections\":[{\"title\":\"⚡ 𝗣𝗹𝘂𝗴𝗶𝗻𝘀 𝗟𝗶𝘀𝘁 ⚡\",\"highlight_label\":\"${Config.botname}\",\"rows\":[{\"header\":\"🍓 Delete All Plugins 🍓\",\"title\":\"𝘤𝘭𝘪𝘤𝘬 𝘩𝘦𝘳𝘦 𝘁𝗼 𝘀𝗲𝘁 𝘁𝗵𝗲 𝗱𝗲𝗹𝗲𝘁𝗲 𝘄𝗶𝘁𝗵 𝘁𝗵𝗲 𝗿𝗲𝗺𝗼𝘃𝗲 𝗮𝗹𝗹 𝘀𝘁𝗮𝘁𝘂𝘀\",\"description\":\"°•~━━✥ Queen Anya MD ✥━━~•°\",\"id\":\"${prefix}bulkplugindelete ${links}\"}]},{\"title\":\"⚡ 𝗣𝗹𝘂𝗴𝗶𝗻𝘀 𝗟𝗶𝘀𝘁 ⚡\",\"highlight_label\":\"${Config.botname}\",\"rows\":[${pluginlist}]}]}`
            }],

...
anya({
    ...
    async (anyaV2, pika, { args, prefix }) => {
        ...
        if (!/^https:\/\/gist\.githubusercontent\.com\/PikaBotz\/.+\/raw\//.test(args[0])) return pika.reply("_❌Invalid URL._");
        const external = await Plugins.find({});
...
async (anyaV2, pika, { args, prefix }) => {
    ...
    if (!/^https:\/\/gist\.githubusercontent\.com\/.+\/raw\//.test(args[0])) return pika.reply("_❌Invalid URL._");
    const external = await Plugins.find({});
    const plugins = external.map(v => v.url);
...
async (anyaV2, pika, { args, prefix }) => {
    ...
    const reply = [];
    const invalidUrls = args.filter(url => !/^https:\/\/gist\.githubusercontent\.com\/PikaBotz\/.+\/raw\//.test(url));

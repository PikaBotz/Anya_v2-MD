...
const { anya, Plugins } = require('../lib');
...
anya({
    ...
    async (anyaV2, pika, { args, prefix }) => {
        if (!args[0]) return pika.reply(`Enter a plugin *url*, type \`${pregi}pluginstore\` to install plugins..!`);
        if (!/^https:\/\/gist\.github\.com\//.test(idOrUrl)) return pika.reply("_❌Invalid Url._");
        const external = Plugins.find({});

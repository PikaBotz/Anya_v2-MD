...
const { anya, commands, UI, Plugins, installPlugins, deletePlugins, getBuffer } = require('../lib');
...
anya({ name: "plugins", alias: ['plugin'], react: "🚀", category: "core", desc: "See all plugins list", rule: 1, filename: __filename },
    async (anyaV2, pika, { args, prefix }) => {
        ...
        const thumbUrl = "https://i.ibb.co/wcxrZVh/hero.png";
        const ui = await UI.findOne({ id: "userInterface" }) || (await new UI({ id: "userInterface" }).save());
        if (ui.buttons) return await anyaV2.sendButtonImage(pika.chat, { caption: caption, image: await getBuffer(thumbUrl), footer: Config.footer,
            buttons: [{
                "name": "single_select",
                "buttonParamsJson": `{"title":"Choose to", "sections":[{"title":"🧾 Plugins List 🧾", "highlight_label":"${Config.botname}",
                    "rows":[{"header":"Plugin 1", "title":"click to delete", "description":"Example description", "id":"${prefix}ping"},
                    {"header":"Plugin 2", "title":"click to delete", "description":"Example description", "id":"${prefix}ping"},
                    {"header":"Plugin 3", "title":"click to delete", "description":"Example description", "id":"${prefix}ping"},
                    {"header":"Plugin 4", "title":"click to delete", "description":"Example description", "id":"${prefix}ping"}]
                }]}`
            }],
            contextInfo: { mentionedJid: [pika.sender] } }, { quoted: pika });
        else return await anyaV2.sendMessage(pika.chat, { text: caption, contextInfo: { externalAdReply: { showAdAttribution: true, title: `Installed Plugins List`, body: 'Reply with a number to delete that plugin', thumbnailUrl: thumbUrl, mediaType: 1, renderLargerThumbnail: true } } }, { quoted: pika });
});
...
anya({ name: "pluginstore", alias: ['pluginsstore'], react: "🍓", category: "core", desc: "External Plugins url and info store", filename: __filename }, async (anyaV2, pika, { args }) => {

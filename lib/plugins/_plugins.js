...
anya({
    ...
    .then(({data})=> {
        ...
        const storelist = data.map((item, index) => "*" + Config.themeemoji + "Url: (" + index + 1 + "):* " + item.url + "*🌟File:* " + item.name + "*👣Type:*" + item.type + "*🏖️About:*" + item.desc).join('\n\n┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n\n');
        return pika.reply(message + storelist + "> " + Config.footer);
    })
    .catch(err=>{

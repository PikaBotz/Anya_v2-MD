...
console.log(allFiles2);
            ...
            commands = commands.filter(cmd => cmd.filename !== response.filename);

            if (commands.length !== originalLength) {
                pika.reply("*☑️Plugin(s) Deleted!*");
            } else {
                pika.reply("_❌No matching plugin commands found in memory._");
            }
//            });
//        } catch (error) {

 ...
 */
const { proto, areJidsSameUser, generateWAMessage, MessageType, newMessagesDB, prepareMessage } = require('@queenanya/baileys');
...
if (pika.msg.fileSha256) {
   ...
   //     const { command, mentions } = cmd.setcmd.get(pika.msg.fileSha256.toString('base64'));
    /*    const text = command;
        const mentionedJid = mentions;
        anyaV2.ev.emit('chat-update', {
            ...chatUpdate,
            messages: newMessagesDB([
                pika.cMod(pika.chat,
                    await prepareMessage(pika.chat, text, MessageType.extendedText, {
                        contextInfo: {
                            mentionedJid
                        },
    ...
    */
        const { command, mentions } = cmd.setcmd.get(pika.msg.fileSha256.toString('base64'));
        const messages = await generateWAMessage(pika.chat, { text: command, mentions: mentions }, {
        ...
        const check = await anyaV2.ev.emit('messages.upsert', msg);

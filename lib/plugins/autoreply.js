...
anya({ usage: "text", notCmd: true }, async (anyaV2, pika, { system, bot }) => {
    if (!pika.text || /* bot.worktype === "self" || */ !system.chatbot) return;
...
anya({ usage: "text", notCmd: true }, async (anyaV2, pika, { group, bot, args, botNumber }) => {
    if (!pika.text || /* bot.worktype === "self" || */ !group.chatbot) return;
    const nullMessage = [
    ...
    if (args.length < 1) return sendAiResponse(pickRandom(nullMessage));
    const userId = pika.sender.split("@")[0];

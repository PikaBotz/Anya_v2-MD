...
> *▢* Used _*${memInfo.usedMemory}*_ out of _*${memInfo.totalMemory}*_ memory.`;
        ...
        switch (ui.alive) {
            ...
            case 10:
                return anyaV2.sendMessage(pika.chat, {
                    video: Config.aliveMedia,
                    caption: messageOptions.caption,
                    gifPlayback: ui.alive === 9 || ui.alive === 10,
                    mentions: messageOptions.mentions,
                    contextInfo: {
                        forwardingScore: ui.alive === 10 ? 999 : undefined,
                        isForwarded: ui.alive === 10,
                        mentionedJid: messageOptions.mentions,
                        forwardedNewsletterMessageInfo: ui.alive === 10 ? {
            ...
            case 11:
                return anyaV2.sendButtonImage(pika.chat, {
                    ...
                    buttons: [
                        { name: "cta_url", buttonParamsJson: `{"display_text":"Instagram 🦋","url":"https://instagram.com/${Config.instagramId}","merchant_url":"https://www.google.com"}` },
                        { name: "cta_url", buttonParamsJson: `{"display_text":"WhatsApp Channel 🔮","url":"https://whatsapp.com/channel/0029VaDIPkA6buMS9hRE7y2G","merchant_url":"https://whatsapp.com/channel/0029VaDIPkA6buMS9hRE7y2G"}` },
                        { name: "cta_url", buttonParamsJson: `{"display_text":"YouTube Channel 💗","url":"https://youtube.com/@pika_kunn","merchant_url":"https://youtube.com/@pika_kunn"}` },
...
┃${symbols}│ Time : ${calender.time}
┃${symbols}│ Plugins : ${commands.length}
┃${symbols}│ Version : v${packageJson.version}
┃${symbols}│ Ram : ${memoryInfo.usedMemory}/${memoryInfo.totalMemory}
...
╰══════════════════─❃\`\`\`\n\n`;
        ...
        const sendMenu = async ({ isCategory, middle = "", buttons = [] }) => {
            const baseMessage = {
                caption: caption + (isCategory ? replyTag + "\n\n" : "") + middle + (isCategory ? "\n\n" + footerTag : ""),
                mentions: [pika.sender],
                quoted: pika
            };
            switch (ui.listmenu) {
                case 1:
                    return pika.reply(caption + (isCategory ? replyTag + "\n\n" : "") + middle + (isCategory ? "\n\n" + footerTag : ""), { mentions: [pika.sender] });
                case 2:
                ...
                case 3:
                    return await anyaV2.sendMessage(pika.chat, {
                        text: caption + (isCategory ? replyTag + "\n\n" : "") + middle + (isCategory ? "\n\n" + footerTag : ""),
                        mentions: [pika.sender],
                        contextInfo: {
                            externalAdReply: {
                                showAdAttribution: true,
        ...
        if (args.length > 0) {
            ...
            if (patterns[category]) {
                ...
                for (const cmd of patterns[category]) {
                    categoryMessage += `│⊳ ${cateSymbol || "🪅"} ${prefix + cmd}\n`;
                }
                categoryMessage += `└───────────────⟢`;
                return sendMenu({ middle: categoryMessage, buttons: [
					{
						"name": "quick_reply",
						"buttonParamsJson": `{"display_text":"Owner 👤","id":"${prefix}owner"}`
					},

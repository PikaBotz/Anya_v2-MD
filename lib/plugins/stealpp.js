...
async function getCommand(userOwner, userSudo, botNumber, anyaV2, pika) {
require('../../config');
    if (!userOwner && !userSudo) return pika.reply(message.owner);
    if (!text && !pika.quoted) return pika.reply("Tag or mention a user to steal his/her profile picture.");
    if (pika.quoted.sender.includes(botNumber)) return pika.reply("Don't tag this bot's own message!");
  try {

...
anya(
    {
        ...
        need: "onceview",
        category: "convert",
        desc: "Convert view once messages into standard medias",
        filename: __filename
    },
    async (anyaV2, pika, { prefix, command }) => {
        const quoted = pika.quoted;
        if (!quoted) return pika.reply(`_Tag a once view message with the caption \`${prefix + command}\`_`);
        console.log(quoted);
        console.log(quoted.mtype);
        //if (!quoted.message.viewOnce) return pika.reply("_It's not an once view message baka ❗_");
        const { key } = await pika.keyMsg(Config.message.wait);
        const type = quoted.message.imageMessage ? "image" : "video";
        const extension = type === "image" ? ".jpg" : ".mp4";
        const buffer = await anyaV2.downloadAndSaveMediaMessage(quoted.message[type + "Message"], getRandom(8) + extension);
        console.log(buffer);
        const caption = quoted.message[type + "Message"]?.caption || "~no caption~";
        await anyaV2.sendMessage(pika.chat, {
            [type]: { url: buffer },
            caption: `${Config.themeemoji} *Caption:* ${caption}`
        }, { quoted: pika });

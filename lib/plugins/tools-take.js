module.exports = {
    ...
    getCommand: async (anyaV2, pika, prefix, text, command) => {
      ...
      if (/webp/.test(mime)) {
        ...
        await writeExifInVid(path + '.webp', { packname: packname, author: (author === '@pikanull') ? null : author })
          ...
          axios.post(dev.api.apiHubRaw + `/api/sticker.js?key=${dev.api.apiHubKey}&packname=${packname}&author=${author}`, { media: image.toString('base64') }, {
            headers: {

...
anya({
    ...
    try {
        const info = await youtube.ytUrlInfo(args[0]);
        console.log(info)
...
`;
            ...
            for (const i of activeQualities) {
                buttonsArray.push(`{\"header\":\"${Config.themeemoji} ${i.quality}\",\"title\":\"\",\"description\":\"${/2160|1440|1080|720/.test(i.quality) ? "⚠️ don't select if bot has low RAM!" : "click here to download this quality"}\",\"id\":\"${prefix}ytv2 ${info.url}\"}`);
            }
            for (const i of activeQualities) {
                buttonsArray.push(`{\"header\":\"${Config.themeemoji} ${i.quality}\",\"title\":\"𝘥𝘰𝘤𝘶𝘮𝘦𝘯𝘵\",\"description\":\"${/2160|1440|1080|720/.test(i.quality) ? "⚠️ don't select if bot has low RAM!" : "click here to download this quality"}\",\"id\":\"${prefix}ytvdoc ${info.url}\"}`);
            }
                buttonsArray.push(`{\"header\":\"${Config.themeemoji} audio/mp3\",\"title\":\"\",\"description\":\"click here to download this audio\",\"id\":\"${prefix}yta2 ${info.url}\"}`);
                buttonsArray.push(`{\"header\":\"${Config.themeemoji} audio\",\"title\":\"𝘥𝘰𝘤𝘶𝘮𝘦𝘯𝘵\",\"description\":\"click here to download this document\",\"id\":\"${prefix}ytadoc ${info.url}\"}`);
            ...
            for (const i of activeQualities) {
                caption += "*" + num++ + " " + symbols + " " + i.quality + "*" + (/2160|1440|1080|720/.test(i.quality) ? "\n_⚠️ don't select if bot has low RAM!_\n\n" : "\n");
            }
            for (const i of activeQualities) {
                caption += "*" + num++ + " " + symbols + " " + i.quality + "* _(document)_" + (/2160|1440|1080|720/.test(i.quality) ? "\n_⚠️ don't select if bot has low RAM!_\n\n" : "\n");
            }
            caption += "*" + (num++) + " " + symbols + " audio/mp3*\n";
            caption += "*" + (num++) + " " + symbols + " audio* _(document)_\n\n";
            caption += "> " + info.videoId + "\n";
            caption += "> _ID: QA34_";
            return await anyaV2.sendMessage(pika.chat, {

...
anya({
    ...
    try {
        const text = args.join(" ");
        const { url } = await youtube.search(text, "videos");
        const info = await youtube.downloadYtVideo(url);
        if (!info.status) return pika.reply("_❌No video Found!_");
        const ui = await UI.findOne({ id: "userInterface" }) || (await new UI({ id: "userInterface" }).save());
        if (/360p/.test(args[1])) {
...
`;
                return await anyaV2.sendButtonText(pika.chat, {
                    ...
                    buttons: [
                        {
                            "name": "quick_reply",
                            "buttonParamsJson": `{\"display_text\":\"Video 🎦\",\"id\":\"${prefix}ytv2 ${info.url} 360p\"}`
                        },
                        {
                            "name": "quick_reply",
                            "buttonParamsJson": `{\"display_text\":\"Document 📄\",\"id\":\"${prefix}ytdoc ${info.url}\"}`
                        },
                        {
                            "name": "cta_url",
...
anya({
    ...
    try {
        const text = args.join(" ");
        const { url } = await youtube.search(text, "videos");
        const info = await youtube.downloadYtAudio(url);

...
anya(
    ...
    async (anyaV2, pika, { db, args, prefix, command }) => {
        ...
        if (search.views || download.views) cpt.push(`⌈Vɪᴇᴡs⌋ : ${search.views ? formatNumber(search.views) : formatNumber(download.views)}`);
        if (search.seconds || download.duration) cpt.push(`⌈Dᴜʀᴀᴛɪᴏɴ⌋ : ${search.seconds ? formatRuntime(search.seconds) : formatRuntime(download.duration)}`);
        if (search.ago || download.uploadedOn) cpt.push(`⌈UᴘʟᴏᴀᴅᴇᴅOɴ⌋ : ${search.ago || download.uploadedOn}`);
        if (search.author?.url || download.channel) cpt.push(`⌈Cʜᴀɴɴᴇʟ⌋ : ${search.author?.url?.split("https://youtube.com/")[1] || download.channel}`);
        const caption = `
\`\`\`

...
anya(
    ...
    async (anyaV2, pika, { db, args, prefix, command }) => {
        ...
        const output = await youtube.search(input, "videos"); console.log(output);
...
anya(
        ...
        async (anyaV2, pika, { db, args, prefix, command }) => {
            ...
            const ui = db.UI?.[0] || await new UI({ id: "userInterface" }).save();
            const cpt = [];
            if (info.likes) cpt.push("⌈Lɪᴋᴇs⌋ : " + info.likes);
            if (info.views) cpt.push("⌈Vɪᴇᴡs⌋ : " + info.views);
            if (info.durantion) cpt.push("⌈Dᴜʀᴀᴛɪᴏɴ⌋ : " + info.duration);
            if (info.uploadedOn) cpt.push("⌈UᴘʟᴏᴀᴅᴇᴅOɴ⌋ : " + info.uploadedOn);
            if (info.channel) cpt.push("⌈Cʜᴀɴɴᴇʟ⌋ : " + info.channel);
            if (info.url) cpt.push("⌈Uʀʟ⌋ : " + info.url);
            if (ui.buttons) return anyaV2.sendButtonVideo(pika.chat, {
                video: { url: info.videoUrl },
                caption: `\`\`\`
${info.title ? info.title : "NO_TITLE_FOUND"}

${cpt.join("\n")}
\`\`\``.trim(),
                footer: Config.footer,
                buttons: [{ "name": "cta_url", "buttonParamsJson": `{"display_text":"Get on YouTube","url":"${info.url}","merchant_url":"${info.url}"}` }]
            }, { quoted: pika });

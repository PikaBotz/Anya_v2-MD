...
anya(
        ...
        async (anyaV2, pika, { db, args, prefix, command }) => {
            ...
            if (search.views || download.views) cpt.push("⌈Vɪᴇᴡs⌋ : " + search.views ? formatNumber(views) : download.views);
            if (search.seconds || download.durantion) cpt.push("⌈Dᴜʀᴀᴛɪᴏɴ⌋ : " + search.seconds ? formatRuntime(search.seconds) : download.durantion);
            if (search.ago || download.uploadedOn) cpt.push("⌈UᴘʟᴏᴀᴅᴇᴅOɴ⌋ : " + search.ago ? search.ago : download.uploadedOn);
            if (search.author?.url || download.channel) cpt.push("⌈Cʜᴀɴɴᴇʟ⌋ : " + search.author?.url ? search.author?.url.split("https://youtube.com/")[1] : download.channel);
            //if (download.url) cpt.push("⌈Uʀʟ⌋ : " + download.url);
            if (ui.buttons) return anyaV2.sendButtonVideo(pika.chat, {
                video: await getBuffer(download.videoUrl),
                caption: `\`\`\`
${download.title ? download.title : "NO_TITLE_FOUND"}

${cpt.join("\n")}
\`\`\``.trim(),
                footer: Config.footer,
                buttons: [{ "name": "cta_url", "buttonParamsJson": `{"display_text":"Get on YouTube","url":"${download.url}","merchant_url":"${download.url}"}` }]
            }, { quoted: pika });

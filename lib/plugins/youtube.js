...
anya({
    ...
    const YouTube = require('../lib/ytdl-core');
    if (!YouTube.isYouTubeUrl(args[0])) return pika.reply(`If you want audio/song by search term 📜 then use \`${prefix}song <query>\``);    
    try {
        const res = await YouTube.getAudUrl(args[0]);
        const buffer = await getBuffer(res.thumb.url);
        await anyaV2.sendMessage(pika.chat, {
            image: buffer,
            caption: `\`${res.title}\`\n
𝘙𝘦𝘱𝘭𝘺 𝘢 𝘯𝘶𝘮𝘣𝘦𝘳 𝘵𝘰 𝘥𝘰𝘸𝘯𝘭𝘰𝘢𝘥
...
\`VID: ${res.id}\``,
        ...
        console.error(error);
        pika.reply("An error occurred while fetching the audio.");
    }
});
...
anya({
    name: "songaudiocommand",
    react: "💃🏻",
    need: "url",
    category: "download",
    desc: "Internal command",
    cooldown: 8,
    notCmd: true,
    filename: __filename
}, async (anyaV2, pika, { args, prefix, command }) => {
    ...
    const YouTube = require('../lib/ytdl-core');
    if (!YouTube.isYouTubeUrl(args[0])) return pika.reply(`If you want audio/song by search term 📜 then use \`${prefix}song <query>\``);    
    const loading = await pika.keyMsg(`\`\`\`downloading audio...\`\`\``);
    try {
        const res = await YouTube.getAudUrl(args[0]);
        const buffer = await getBuffer(res.thumb.url);
        await anyaV2.sendMessage(pika.chat, {
...
anya({
    ...
    const YouTube = require('../lib/ytdl-core');
    if (!YouTube.isYouTubeUrl(args[0])) return pika.reply(`If you want audio/song by search term 📜 then use \`${prefix}song <query>\``);    
    const loading = await pika.keyMsg(`\`\`\`downloading doc...\`\`\``);
    try {
        const res = await YouTube.getAudUrl(args[0]);
        const buffer = await getBuffer(res.thumb.url);
        await anyaV2.sendMessage(pika.chat, {

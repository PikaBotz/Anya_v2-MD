...
`;
     ...
     */
      switch (args[0]) {
        case 'autostatus':
          Bot.set(pika.chat, 'autoStatusRead', /enable/.test(command) ? true : false)
          .then((res) => response(res.status, `_Already ${command}d automatic status seen._`));
          break;
        case 'alwaysonline':
          Bot.set(pika.chat, 'alwaysOnline', /enable/.test(command) ? true : false)
          .then((res) => response(res.status, `_Already ${command}d always online seen for bot._`));
          break;
        case 'autoread':
          Bot.set(pika.chat, 'autoMsgRead', /enable/.test(command) ? true : false)
          .then((res) => response(res.status, `_Already ${command}d automatic message read._`));
          break;
        case 'autotyping':
          Bot.set(pika.chat, 'autoTyping', /enable/.test(command) ? true : false)
          .then((res) => response(res.status, `_Already ${command}d automatic typing while replying._`));
          break;
        case 'autoreact':
          Bot.set(pika.chat, 'autoReactMsg', /enable/.test(command) ? true : false)

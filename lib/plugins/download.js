    ...
    try {
        ...
        for (let i = 0; i < response.length; i++) {
            await anyaV2.sendFileUrl(pika.chat, {
                url: response[i].url_download,
                caption: `> Downloaded from Instagram`,
                mime: response[i].type
            }, { quoted:pika });
        }
    } catch (error) {
        pika.deleteMsg(key);
...
anya(
    ...
    async (anyaV2, pika, { args }) => {
        ...
        if (!repoInfo || response.status !== 200) return pika.reply("_❌Unable to find the repository. Please ensure the URL is correct._");
        pika.reply("✅ Found one repo, _*${(repoInfo?.size / 1024).toFixed(2)}MB*_ size.");
        await anyaV2.sendMessage(pika.chat, {

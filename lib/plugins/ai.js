...
const characters = [
	"anya": { name: "anya", alias: [], react: "💅🏻" }
	];

characters.forEach(cai => {
	anya(
		{
			name: cai.name,
			react: cai.react,
		...
		async (anyaV2, pika, { args, command }) => {
			const empty = {
				...
				"Say something why are you quite!",
				"Um nothing to say? 🥲",
				"Ah waiting for you response... 😭"
					]
			};
			if (args.length < 1) return pika.reply(pickRandom(empty[command] || ["Enter a prompt to continue"]));
			switch (command) {
				case "anya":
					cai(pika.sender, args.join(" "), "anya")
					.then(response => {

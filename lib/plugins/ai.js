...
const characters = [
	{ name: "anya", react: "💅🏻" },
	{ name: "elon", react: "🌌" },
	{ name: "characterai", alias: ['cai'], react: "🔖" },
	{ name: "eren", react: "😈" },
	{ name: "mikasa", react: "🗡️" },
	{ name: "raiden", react: "⚡" },
	{ name: "hutao", react: "👻" },
...
characters.forEach(char => {
	anya(
		{
			name: (char.name === 'anya') ? 'anya' : char.name + "ai",
			react: char.react,
			alias: char.alias || [],
			need: "text",
			category: "ai",
			desc: "Chat to " + char.name,
			cooldown: 8,
			filename: __filename
		},
		async (anyaV2, pika, { args, command }) => {
			const emptyResponses = {
				anya: [
					"Hey darling what you want me to talk! 💅🏻",
					"Say something sanõrita! 🥹",
					"Ouu darling tell me wa-happened!",
					"Say something why are you quiet!",
					"Um nothing to say? 🥲",
					"Ah waiting for your response... 😭"
				]
			};
			if (args.length < 1) return pika.reply(pickRandom(emptyResponses[char.name] || ["Enter a prompt to continue"]));
			try {
				const response = await cai(pika.sender, args.join(" "), char.name);
				if (!response.status || !response.message) {
					return pika.reply("```Uh failed to get AI character output!```");
				}
				return pika.reply(response.message);
			} catch (err) {
				return pika.reply("```Error processing AI response: " + err.message + "```");
			}
		}
	);
});

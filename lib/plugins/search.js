...
anya(
	...
	async (anyaV2, pika, { args, prefix, command }) => {
		...
		if (text.includes('https://happymod.com/')) return pika.reply('_Use `' + prefix + command + 'dl <url>` for URLs');
...
`.trim();
			await anyaV2.sendButtonImage(pika.chat, {
				caption: caption,
				image: await getBuffer("https://iili.io/d16FQzx.jpg"),
				...
				contextInfo: {
					externalAdReply: {
						...
						title: `${Config.botname} HAPPYMOD Engine`,
                        			...
                        			thumbnailUrl: "https://iili.io/d16FQzx.jpg",
						mediaType: 1,
...
anya(
	...
	async (anyaV2, pika, { args }) => {
		...
		for (const { title, genre, author, likes, link } of response.results) {
			...
			caption += `_📈 \`Likes:\` ${Number(likes.replace(/,/g, ''))}\n`;
			caption += `_🔗 ${link} ;_\n\n`;
			caption += "•- - - - - - - - - - - - - - - - - - - - - - - - - - - - - -•\n\n";
		}
		caption += `> ${Config.footer}`;

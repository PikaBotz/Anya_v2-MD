...
anya(
	...
	async (anyaV2, pika, { args, prefix, command }) => {
		if (args.length < 1) return pika.reply("_Which app you wanna search ❓_");
	    ...
	    if (response.length < 1 || !response.status) return pika.reply("_❌ No Search Result Found!_");
	    if (ui.buttons) {
			const list = [];
			for (const { link, name, developer, rate_string } of response) {

...
anya(
    ...
    async (anyaV2, pika, { args }) => {
        ...
        const content = pika.quoted ? pika.quoted.text ? pika.quoted.text : args[0] ? text : false : args[0] ? text : false;
        ...
        const metadata = await anyaV2.groupMetadata(info.id)
            .catch(err=> console.log("ERR IN GC METADATA FETCH :", err));
        const botNumber = await anyaV2.decodeJid(anyaV2.user.id);

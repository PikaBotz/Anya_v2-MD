...
anya({
    ...
    const [blocklist, bot, botNumber, userOwner] = await Promise.all([
        anyaV2.fetchBlocklist(),
        Bot.findOne({ id: 'anyabot' }),
        anyaV2.decodeJid(anyaV2.user.id),
        Bot.findOne({ id: 'anyabot' }).then(bot => [
            ...bot.modlist, 
            Config.ownernumber, 
            anyaV2.user.id
...
anya({
    ...
    const botNumber = await anyaV2.decodeJid(anyaV2.user.id);
    const userOwner = new Set([...bot.modlist, Config.ownernumber, botNumber].map(v => v.replace(/[^0-9]/g) + "@s.whatsapp.net"));
    const caption = [];
    ...
    for (const user of users) {
        ...
        if (exist.length < 1) {
            ...
            const userData = await User.findOne({ id: userId }) || await new User({ id: userId }).save();
            if (userData.ban) {
...
anya({
    ...
    const bot = await Bot.findOne({ id: 'anyabot' });
    const botNumber = await anyaV2.decodeJid(anyaV2.user.id);
    const userOwnerSet = new Set([...bot.modlist, Config.ownernumber, botNumber].map(v => v.replace(/[^0-9]/g) + "@s.whatsapp.net"));
    const caption = [];
    for (const user of users) {
        const userId = user.split('@')[0];
        const exist = await anyaV2.onWhatsApp(userId);        
...
anya({
    ...
    if (args.length < 1) return pika.reply(`*${Config.themeemoji} Example:* ${prefix + command} on/off\n\n> Bot will react to command messages.`);
    ...
    if (state === "on") {
        ...
        const ui = await UI.findOne({ id: "userInterface" }) || (await new UI({ id: "userInterface" }).save());
            if (ui.buttons) return await anyaV2.sendButtonText(pika.chat, {
                text: "Choose a option below to turn `on/off, Auto Command React`",
                footer: Config.footer,
                buttons: [
                        { "name": "quick_reply", "buttonParamsJson": `{\"display_text\":\"Turn On ✅\",\"id\":\"${prefix + command} on\"}` },
                        { "name": "quick_reply", "buttonParamsJson": `{\"display_text\":\"Turn Off ❌\",\"id\":\"${prefix + command} off\"}` },
...
anya({
    ...
    if (state === "on") {
        ...
        const ui = await UI.findOne({ id: "userInterface" }) || (await new UI({ id: "userInterface" }).save());
            if (ui.buttons) return await anyaV2.sendButtonText(pika.chat, {
                text: "Choose a option below to turn `on/off, Auto Message React`",
                footer: Config.footer,
                buttons: [
                        { "name": "quick_reply", "buttonParamsJson": `{\"display_text\":\"Turn On ✅\",\"id\":\"${prefix + command} on\"}` },
                        { "name": "quick_reply", "buttonParamsJson": `{\"display_text\":\"Turn Off ❌\",\"id\":\"${prefix + command} off\"}` },
...
anya({
    ...
    if (state === "on") {
        ...
        const ui = await UI.findOne({ id: "userInterface" }) || (await new UI({ id: "userInterface" }).save());
            if (ui.buttons) return await anyaV2.sendButtonText(pika.chat, {
                text: "Choose a option below to turn `on/off, Auto Status Seen`",
                footer: Config.footer,
                buttons: [
                        { "name": "quick_reply", "buttonParamsJson": `{\"display_text\":\"Turn On ✅\",\"id\":\"${prefix + command} on\"}` },
                        { "name": "quick_reply", "buttonParamsJson": `{\"display_text\":\"Turn Off ❌\",\"id\":\"${prefix + command} off\"}` },
...
anya({
    ...
    const userPromises = users.map(async i => {
        ...
        if (exist.length < 1) {
            ...
            const res = await addWarn(userId, { chat: isPrivateChat ? 1 : 2, reason });
...
anya({
    ...
    const userPromises = users.map(async i => {
        ...
        if (exist.length < 1) {
            ...
            const res = await delWarn(userId, { chat: isPrivateChat ? 1 : 2, reason });

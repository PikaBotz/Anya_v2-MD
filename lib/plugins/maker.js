...
const textpro = [
    'candy', 'nail', 'moss', 'shadow', 'iphone15',
    'pokemon', 'naruto', 'liquid', 'party', 'bornpink',
    'typography', 'neon', 'neon2', 'neon3', 'neon4',
    'blackpink', 'blackpink2', 'sliced', 'batman',
    'valentine', 'demon', 'magma', 'glitch', 'glitch2',
    'harrypotter', 'neondevil', 'matrix', 'thunder', 'road',
    'bokeh', 'glow', 'wall', 'ice', 'deepsea', 'scifi',
    'rainbow', 'pipe', 'halloween', 'sketch', 'circuit',
    'space', 'metal', 'transformer', 'berry', 'papercut',
    'watercolor', 'underwater', 'graffiti', 'honey', 'juice',
    'biscuit', 'wood', 'choco', 'blood', 'toxic', 'sand',
    'glue', 'sunset', 'leaf'
];

textpro.forEach(style => {
    anya({
        name: style,
        react: "✨",
        need: "text",
        category: "textpro",
        desc: "Make text images using texts",
        cooldown: 8,
        filename: __filename
    },
    async (anyaV2, pika, { args, prefix, command }) => {
        if (args.length < 1) return pika.reply(`Example: ${prefix + command} PikaBotz`);
        const { key } = await pika.keyMsg("```Creating...```");
        pikaApi.get("api", "textpro", `style=${command}&q=${encodeURIComponent(args.join(" "))}`)
        .then(async res => {
            if (!res.results) return pika.edit(Config.message.error, key);
            await anyaV2.sendMessage(pika.chat,
                {
                    image: await getBuffer(res.results),
                    caption: tiny("Textpro image by " + Config.botname)
                },
                {
                    quoted: pika
                })

...
anya({
    ...
    .then(async response => {
        const caption = `
✘ Uꜱᴇʀɴᴀᴍᴇ » @${response.username}
✘ Nᴀᴍᴇ » ${response.name || "N/A"}
✘ Cᴀᴛᴇɢᴏʀʏ » ${response.category || "N/A"}
✘ Pᴏꜱᴛꜱ » ${response.posts || "N/A"}
✘ Pʀᴏɴᴏᴜɴꜱ » ${(response?.pronouns && response.pronouns.length > 0) ? response.pronouns.join(", ") : "N/A"}
✘ Fᴏʟʟᴏᴡᴇʀꜱ » ${response.followers || "N/A"}
✘ Fᴏʟʟᴏᴡɪɴɢ » ${response.following || "N/A"}
✘ ʙɪᴏ »
${response.bio || "N/A"}
        `.trim();        
    ...
    .catch(async err => {
        ...
        // Fallback to Guru API if Ironman API fails

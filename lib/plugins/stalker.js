...
anya({
    ...
    .then(response => {
        ...
        const cpt = [];
        const others = [];
        if (response.is_private !== 'unknown' && response.is_private === true) {
            cpt.push('```⚠️ Account Is Private..!\n```');
            cpt.push('*❖ Username :* ' + response.username);
            cpt.push('*❖ Fullname :* ' + response.fullname);
            cpt.push('*❖ Followers :* ' + formatNumber(response.followers));
            cpt.push('*❖ Followers :* ' + formatNumber(response.followings));
            if (response.posts) cpt.push('*❖ Posts :* ' + formatNumber(response.posts));
            //if (response.igtv_posts) cpt.push('*❖ IgTv Posts :* ' + formatNumber(response.igtv_posts));
        }
        const pfpUrl = response.profile_picture || Config.imageUrl;
        ...
        anyaV2.sendMessage(pika.chat, {
            ...
            contextInfo: {
                externalAdReply: {
                    ...
                    title: `${response.fullname} (${response.username}) • Instagram Profile`,
                    body: `${response.followers} Followers, ${response.followings} Following${response.posts ? `, ${response.posts} Posts` : ''} • @PikaBotz Inc.`,
                    thumbnailUrl: pfpUrl,
                    mediaType: 1,

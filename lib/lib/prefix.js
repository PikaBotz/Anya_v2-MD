exports.getPrefix = async function (body, mode) {
    ...
    if (mode === 'all') {
        var prefix = (() => {
            if (symbolsRegex.test(body)) {
                ...
                return '';
            } else if (Config.prefa && !body.startsWith(Config.prefa)) {
                return '';
            } else {
                return Config.prefa || '';
            }
     ...
     */
        ...
        var prefix = (() => {
            if (typeof body === 'string' && symbolsRegex.test(body)) {
                ...
                return Config.prefa;
            } else if (Config.prefa && typeof body === 'string' && !body.startsWith(Config.prefa)) {
                return Config.prefa;
            } else {
                return Config.prefa;
            }
    ...
    return {
        ...
        isCmd: body.startsWith("@") ? false : isCmd,
        command: command
    }
}
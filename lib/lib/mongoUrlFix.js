...
module.exports = {
  fix: (url) => {
    ...
    if (urlPattern.test(url)) {
      const correctedUrl = `${url.replace(/<|>/g, '')}/Anya_v2_database?retryWrites=true&w=majority`;
      return { status: /* Url corrected */ 200, url: correctedUrl, message: 'Attempting to try to correct the URL proceed successfully.' };
    } else {
      const matchPassword = /mongodb\+srv:\/\/well:([^@]+)@well\.wxgbzs7\.mongodb\.net\/?$/;
      const match = url.match(matchPassword);
      if (match && match[1]) {
        const correctedUrl = `${url.replace(match[1], encodeURIComponent(match[1]))}/Anya_v2_database?retryWrites=true&w=majority`;

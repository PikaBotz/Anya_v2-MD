 ...
 * @author: https://github.com/PikaBotz
 */
exports.stickerToVideo = (stickerBuffer) => {
    return new Promise((resolve, reject) => {
        const directory = path.join(__dirname, '../../.temp');
        const tempStickerPath = directory + getRandom(8) + ".webp";
        const tempVideoPath = directory + getRandom(8) + ".mp4";
        fs.writeFileSync(tempStickerPath, stickerBuffer);
        ffmpeg(tempStickerPath)
            .outputOptions('-c:v libx264', '-pix_fmt yuv420p', '-movflags frag_keyframe+empty_moov+faststart')
            .toFormat('mp4')
            .on('end', () => {

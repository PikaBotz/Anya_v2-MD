...
require('../../config');
  ...
  const _groupData = require('../database/groups/' + m.chat + '.json');
  const { fetchJson, formatp, runtime } = require('../lib/myfunc.js');
  ...
  if (!m.isGroup) {
    txt += "*🎗️ Role :* Private Chat\n\n";
  } else {
    txt += `*🎗️ Role :* ${isAdmins? "Admin 👑️":"Member 👤"}\n\n`;
  }
  txt += "*>>> 🤖 _BOT INFO_*\n";
  txt += `❲❒❳ *Name :* ${botname}\n`;

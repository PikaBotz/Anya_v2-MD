...
async function getCommand(react, text, args, AnyaPika){
require('../../config');
  const { ssweb } = require('api-dylux');
  if (!text) return m.reply("Enter a website URL to stalk.");
  if (!text.includes("https://")) return m.reply("Please enter a valid URL.");
  m.reply(mess.wait);
  try {
